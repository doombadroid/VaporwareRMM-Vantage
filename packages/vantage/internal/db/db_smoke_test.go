...
func TestInit_AcceptsValidPostgresURL_AndAppliesMigrations(t *testing.T) {
	...
	t.Cleanup(func() {
		if DB != nil {
			...
			_, _ = DB.Exec(`DROP TABLE IF EXISTS enrollment_tokens, vantage_signing_key, tailscale_connection, audit_log, user_sessions, users, edges, schema_migrations CASCADE`)
			_ = DB.Close()
			DB = nil
		}
	})
