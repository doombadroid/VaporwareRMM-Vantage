...
func setup(t *testing.T) {
	...
	_, _ = conn.Exec(`DROP TABLE IF EXISTS command_queue, tags, tag_endpoint_membership, audit_checkpoints, tailscale_connection, vantage_signing_key, enrollment_tokens, audit_log, user_sessions, users, edges, schema_migrations CASCADE`)
	_ = conn.Close()
	t.Cleanup(func() {
		if db.DB != nil {
			_, _ = db.DB.Exec(`DROP TABLE IF EXISTS command_queue, tags, tag_endpoint_membership, audit_checkpoints, tailscale_connection, vantage_signing_key, enrollment_tokens, audit_log, user_sessions, users, edges, schema_migrations CASCADE`)
			_ = db.DB.Close()
			db.DB = nil
		}
		ResetForTests()
